...
class ScrapCollectionRecordService {
     ...
     */
    async getFormHelpers(collectorId) {
        try {
            // Get assigned work orders for this collector
            const workOrders = await config_1.prisma.order.findMany({
                where: {
            ...
            const scrapCategories = await config_1.prisma.scrap_categories.findMany({
                where: {
            ...
            // Get all scrap names
            const scrapNames = await config_1.prisma.scrap_names.findMany({
                where: {
                    organizationId: collector.organizationId,
                    isActive: true
                },
            ...
            return api_result_1.ApiResult.success(helpers, 'Form helpers retrieved successfully');
        }
        catch (error) {
            console.error('Error in getFormHelpers:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to fetch form helpers', 500);
        }
    }
    /**
     * Create a new scrap collection record
     */
    async createRecord(collectorId, data) {
        try {
            // Get collector's organization
            const collector = await config_1.prisma.employee.findUnique({
                where: { id: collectorId },
            ...
            // Create the record
            const record = await config_1.prisma.scrap_collection_records.create({
                data: {
                    ...
                    scrapCondition: data.scrapCondition,
                    make: data.make,
                    model: data.model,
                    ...
                    collectionStatus: enum_1.CollectionRecordStatus.SUBMITTED
                },
                select: {
                    id: true
                }
            });
            return api_result_1.ApiResult.success({ id: record.id }, 'Collection record created successfully', 201);
        }
        catch (error) {
            console.error('Error in createRecord:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to create collection record', 500);
        }
    }
    /**
     * Get collection records with filtering
     */
    async getRecords(collectorId, query) {
        try {
            const { page = 1, limit = 20, collectionStatus, workOrderId, customerId, scrapCategoryId, dateFrom, dateTo, search, sortBy = 'createdAt', sortOrder = 'desc' } = query;
            ...
            if (dateFrom || dateTo) {
                ...
                if (dateFrom)
                    where.collectionDate.gte = new Date(dateFrom);
                if (dateTo)
                    where.collectionDate.lte = new Date(dateTo);
            }
            // Search filter
            if (search) {
                where.OR = [
            ...
            const [records, total] = await Promise.all([
                config_1.prisma.scrap_collection_records.findMany({
                    where,
                    skip,
                    take: parsedLimit,
                    include: {
                ...
                config_1.prisma.scrap_collection_records.count({ where })
            ]);
            // Calculate summary
            const summary = {
                totalRecords: total,
                totalAmount: await config_1.prisma.scrap_collection_records.aggregate({
                    where,
                    _sum: { finalAmount: true }
                }).then(result => result._sum.finalAmount || 0),
                submittedCount: await config_1.prisma.scrap_collection_records.count({
                    where: { ...where, collectionStatus: enum_1.CollectionRecordStatus.SUBMITTED }
                })
            };
            const response = {
                records,
            ...
            return api_result_1.ApiResult.success(response, 'Collection records retrieved successfully');
        }
        catch (error) {
            console.error('Error in getRecords:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to fetch collection records', 500);
        }
    }
    /**
     * Get collection records by work order ID (for dashboard/admin access)
     */
    async getRecordsByWorkOrder(workOrderId) {
        try {
            const records = await config_1.prisma.scrap_collection_records.findMany({
                where: {
                    workOrderId
                },
                include: {
            ...
            return api_result_1.ApiResult.success({ records }, 'Collection records retrieved successfully');
        }
        catch (error) {
     ...
     */
    async getRecordById(collectorId, recordId) {
        try {
            const record = await config_1.prisma.scrap_collection_records.findFirst({
                where: {
                    id: recordId,
                    collectorId
                },
            ...
            if (!record) {
                return api_result_1.ApiResult.error('Collection record not found or not authorized', 404);
            }
            return api_result_1.ApiResult.success(record, 'Collection record retrieved successfully');
        }
        catch (error) {
            console.error('Error in getRecordById:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to fetch collection record', 500);
        }
    }
    /**
     * Update a collection record
     */
    async updateRecord(collectorId, recordId, data) {
        try {
            // Verify record exists and belongs to collector
            const existingRecord = await config_1.prisma.scrap_collection_records.findFirst({
                where: {
                    id: recordId,
                    collectorId
                }
            ...
            const record = await config_1.prisma.scrap_collection_records.update({
                where: { id: recordId },
                data: updateData,
                include: {
                    scrap_categories: true,
            ...
            return api_result_1.ApiResult.success(record, 'Collection record updated successfully');
        }
        catch (error) {
            console.error('Error in updateRecord:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to update collection record', 500);
        }
    }
    /**
     * Delete a collection record
     */
    async deleteRecord(collectorId, recordId) {
        try {
            const record = await config_1.prisma.scrap_collection_records.findFirst({
                where: {
                    id: recordId,
            ...
            await config_1.prisma.scrap_collection_records.delete({
                where: { id: recordId }
            });
            return api_result_1.ApiResult.success(null, 'Collection record deleted successfully');
        }
        catch (error) {
            console.error('Error in deleteRecord:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to delete collection record', 500);
        }
    }
    /**
     * Generate PDF for a scrap collection record
     */
    async generatePDF(collectorId, recordId) {
        try {
            const record = await config_1.prisma.scrap_collection_records.findFirst({
                where: {
                    id: recordId,
            ...
            return api_result_1.ApiResult.success({ html, record }, 'PDF data generated successfully');
        }
        catch (error) {
            console.error('Error in generatePDF:', error);
            return api_result_1.ApiResult.error(error.message || 'Failed to generate PDF', 500);
        }
    }
    /**
     * Get PDF data by record ID (for dashboard/admin access)
     */
    async getPDFDataById(recordId) {
        try {
            const record = await config_1.prisma.scrap_collection_records.findFirst({
                where: { id: recordId },
                include: {
            ...
            if (!record) {
                return api_result_1.ApiResult.error('Collection record not found', 404);
            }
            // Generate HTML for PDF
            const html = this.generatePDFHTML(record);
            return api_result_1.ApiResult.success({ html, record }, 'PDF data generated successfully');
            ...
            return api_result_1.ApiResult.error(error.message || 'Failed to generate PDF data', 500);
        }
    }
    /**
...
<head>
    <meta charset="UTF-8">
    <title>Scrap Collection Record - ${((_a = record.Order) === null || _a === void 0 ? void 0 : _a.orderNumber) || record.id}</title>
    <style>
...
<body>
    ...
    <table><thead><tr><th colspan="2">📋 WORK ORDER INFORMATION</th></tr></thead><tbody>
        <tr>
            <td style="width: 50%;"><div class="label">WORK ORDER NUMBER</div><div class="value">${((_b = record.Order) === null || _b === void 0 ? void 0 : _b.orderNumber) || 'N/A'}</div></td>
        ...
        <tr>
            ...
            <td><div class="label">COLLECTOR</div><div class="value">${((_c = record.Employee) === null || _c === void 0 ? void 0 : _c.fullName) || 'N/A'}</div></td>
        </tr>
    </tbody></table>

    <table><thead><tr><th colspan="2">👤 CUSTOMER INFORMATION</th></tr></thead><tbody>
        <tr>
            <td style="width: 50%;"><div class="label">CUSTOMER NAME</div><div class="value">${((_d = record.Customer) === null || _d === void 0 ? void 0 : _d.name) || 'N/A'}</div></td>
            <td style="width: 50%;"><div class="label">PHONE NUMBER</div><div class="value">${((_e = record.Customer) === null || _e === void 0 ? void 0 : _e.phone) || 'N/A'}</div></td>
        </tr>
    </tbody></table>

    <table><thead><tr><th colspan="3">♻️ SCRAP DETAILS</th></tr></thead><tbody>
        <tr>
            <td style="width: 33.33%;"><div class="label">CATEGORY</div><div class="value">${((_f = record.scrap_categories) === null || _f === void 0 ? void 0 : _f.name) || 'N/A'}</div></td>
            <td style="width: 33.33%;"><div class="label">SCRAP NAME</div><div class="value">${((_g = record.scrap_names) === null || _g === void 0 ? void 0 : _g.name) || 'N/A'}</div></td>
        ...
        <tr>
            <td style="width: 50%; text-align: center; padding: 20px; border: 2px dashed #d1d5db; background: #f9fafb;">
                ...
                ${record.customerSignature ? `
                    ...
                    <div style="font-size: 9pt; font-weight: 600; margin-top: 8px;">${((_h = record.Customer) === null || _h === void 0 ? void 0 : _h.name) || ''}</div>
            ...
            <td style="width: 50%; text-align: center; padding: 20px; border: 2px dashed #d1d5db; background: #f9fafb;">
                ...
                ${record.collectorSignature ? `
                    ...
                    <div style="font-size: 9pt; font-weight: 600; margin-top: 8px;">${((_j = record.Employee) === null || _j === void 0 ? void 0 : _j.fullName) || ''}</div>
                ` : '<div style="color: #9ca3af; font-style: italic;">No signature</div>'}
            </td>
        </tr>
    </tbody></table>
...
exports.ScrapCollectionRecordService = ScrapCollectionRecordService;
